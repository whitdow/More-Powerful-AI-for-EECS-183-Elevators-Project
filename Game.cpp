...
bool Game::isValidPickupList(const string& pickupList, 
    ...
    for (int i = 0; i < pickupList.length(); ++i) {
        ...
        if (index >= pickupFloor.getNumPeople()) {
            return false;
        }
    }
    int firstIndex = pickupList[0] - '0';
    bool goingUp = pickupFloor.getPersonByIndex(firstIndex).getTargetFloor() > pickupFloorNum;
    for (int i = 1; i < pickupList.length(); ++i) {
        int index = pickupList[i] - '0';
        bool currentGoingUp = pickupFloor.getPersonByIndex(index).getTargetFloor() > pickupFloorNum;

        if (currentGoingUp != goingUp) {
            return false;
        }
    }
...
bool Game::saveGame(ofstream& saveFile) const {
    ...
    /*
     * At any point, the user can save the game as well. This will write the game’s current state 
     * into another file (called save.in) that can be loaded up even after quitting the game. 
     * The contents of the save-file will look identical to game.in: it will save the current location
     * of all the elevators, the current scoring info, the current time, current people, and the remaining events. 
     * This way, we can start back up where we left off after we load a game.
     */
}
